...
**2026-04-18 업데이트 — ✅ 해결**
- `src/utils/upload.ts::uploadWithTimeout` 도입 (Phase 6 #57).
- `src/services/profile.ts::uploadPhoto` · `src/services/voice.ts::uploadVoiceClone` 모두 래퍼 경유.
- 타임아웃 시 `ApiRequestError(0, 'Upload timeout. Please try again.')` surface → 기존 catch 경로와 호환.
